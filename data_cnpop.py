...
def traverse_dir(root_dir, extension, amount=None, str_include=None, str_exclude=None, is_pure=False, is_sort=False, is_ext=True):
    ...
    Args:
        ...
        amount      :: int - Maximum amount of files
        str_include :: str - Whitelist string in a file name
        str_exclude :: str - Blacklist string in a file name
        is_pure
        is_sort :: boolean - Whether file list should be sorted or not
        is_ext
    Returns:
        file_list :: list[]
    """
...
def get_data_loaders(args, whole_audio=False):
    ...
    Returns:
        ...
        loader_valid - DataLoader for valid
    """
...
class AudioDataset(Dataset):
    def __init__(self, path_root, waveform_sec, hop_size, sample_rate, whole_audio=False):
        super().__init__()
    ...
    def __getitem__(self, file_idx):
        ...
        duration = librosa.get_duration(filename=os.path.join(self.path_root, 'audio', name) + '.wav', sr=self.sample_rate)
    ...
    def get_data(self, name, duration):
        ...
        # Audio - Clipping into N seconds
        waveform_sec = duration if self.whole_audio else self.waveform_sec
        idx_from = 0 if self.whole_audio else random.uniform(0, duration - waveform_sec - 0.1)
        audio, sr = librosa.load(path_audio, sr=self.sample_rate, offset=idx_from, duration=waveform_sec)
        ## Clipping
        ...
        # fo - extraction by DIO
        ## TODO: Preprocessing
        f0, _ = pw.dio(audio.astype('double'), self.sample_rate, f0_floor=65.0, f0_ceil=1047.0, channels_in_octave=2, frame_period=(1000*frame_resolution))
